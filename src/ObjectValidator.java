...
public class ObjectValidator {
    ...
    public List<String> validate(Object target)
        ...
        for (Field field : fields) {
            ...
            if (field.isAnnotationPresent(Limit.class)) {
                allGood = handleLimit(target, field);
                if (!allGood) {
                    results.add("Limit: " + field.getName() +
                            " failed. Failed Value is " + field.get(target));
                }
            }
    ...
    public boolean handleLimit(Object target, Field field) throws IllegalArgumentException, IllegalAccessException {

        Limit uc = (Limit) field.getAnnotation(Limit.class);
        int value = (int) field.get(target);

        int limit = uc.limit();

        System.out.println("limit=" + limit + " value=" + value);
        return value <= limit;
    }
    ...
    public boolean handleLength(Object target, Field field) throws IllegalArgumentException, IllegalAccessException {
        ...
        Length uc = (Length) field.getAnnotation(Length.class);
        ...
        System.out.println("min=" + min + " len=" + length + " max= " + max);
